...
public class Maze extends Canvas {
	...
	private boolean makeMove(int r, int c) {
		//If goal is reached then return true otherwise return false.
		if (board[r][c] == 'G') {
			return true;
		}
		...
		if (board[r][c] == '|' || board[r][c] == 'X' || board[r][c] == '-'|| board[r][c] == '+' ) {
			return false;
		} //That is for to check in four direction in order to move the maze

        //This is for return X if there is any block in forward 
		board[r][c] = 'X';
		boolean result;    

	//These following code is to make the move to up,down,right and left.
		result = makeMove(r, c + 1);
		if (result) {                         //To make the maze move from left to right
			return true;
		}

		result = makeMove(r - 1, c);
		if (result) {                            //To make the maze move from down to up 
			return true;
		}

		result = makeMove(r, c - 1);
		if (result) {                          //To make the maze move from right to left
			return true;
		}

		result = makeMove(r + 1, c);
		if (result) {                        //To make the maze move from up to down
			return true;
		}
		
		board[r][c] = ' ';                   //To leave space where it have visited 
...
public Maze solveMaze(int r, int c){
	...
	if (done(r,c)) {solution.mark[r][c]++; mark[r][c]++;
	solution.board[r][c] = 'X'; board[r][c] = 'X';
